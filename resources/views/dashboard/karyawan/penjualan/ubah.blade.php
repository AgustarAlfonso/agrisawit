        ...
        <div class="col-md-10 mx-auto"> <!-- Centered content -->
            ...
            <div class="mx-auto max-w-screen-xl px-4 py-2 sm:px-6 lg:px-8">
                <div class="mx-auto max-w-lg text-center">
                    <h1 class="text-2xl font-bold sm:text-3xl">Ubah Laporan Penjualan</h1>
                </div>

                <form action="{{ route('dashboard.karyawan.penjualan.ubah.submit', $penjualan->id) }}" method="POST" class="mx-auto mt-8 max-w-md space-y-4">
                    @csrf
                    ...
                    <div>
                        ...
                        <input 
                            ...
                            value="{{ old('jumlahTerjualFormatted', number_format($penjualan->jumlahTerjual, 0, ',', '.')) }}"
                            required>
                        <input 
                            type="hidden" 
                            id="jumlahTerjual" 
                            name="jumlahTerjual" 
                            value="{{ old('jumlahTerjual', $penjualan->jumlahTerjual) }}">
                        @error('jumlahTerjual')
                            <p class="text-sm text-red-500 mt-2">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <!-- Total Harga -->
                    <div>
                        <label for="totalHargaFormatted" class="block text-sm font-medium text-gray-700">Total Harga Penjualan</label>
                        <input 
                            type="text" 
                            id="totalHargaFormatted" 
                            class="w-full rounded-lg p-4 text-sm shadow-sm @error('totalHarga') border-red-500 @enderror" 
                            placeholder="Masukkan total harga penjualan"
                            value="{{ old('totalHargaFormatted', number_format($penjualan->totalHarga, 0, ',', '.')) }}"
                            required>
                        <input 
                            type="hidden" 
                            id="totalHarga" 
