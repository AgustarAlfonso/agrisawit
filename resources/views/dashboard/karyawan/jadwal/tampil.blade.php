     ...
     <div class="row">
         <div class="col-md-10 mx-auto">
            <!-- Adjust column width and offset for sidebar -->
             <h2 class="text-4xl font-extrabold dark:text-white">Jadwal perawatan dan pemupukan</h2>
             
             <!-- Tombol Tambah yang memunculkan Modal -->
             <div class="flex justify-end">
                 <a href="{{ route('dashboard.karyawan.jadwal.tambah') }}">
                 <button
                     class="inline-flex items-center gap-2 rounded-md bg-green-400 px-4 py-2 text-sm text-white shadow-sm hover:bg-green-500 focus:relative mb-4">
             ...
             <div class="overflow-x-auto">
                 <table class="min-w-full divide-y-2 divide-gray-200 bg-white text-sm">
                     <thead>
                         <tr>
                           <th class="whitespace-nowrap px-4 py-2 font-medium text-gray-900 w-40">Jenis</th>
                           <th class="whitespace-nowrap px-4 py-2 font-medium text-gray-900 w-40">Tanggal  </th>
                           <th class="whitespace-nowrap px-4 py-2 font-medium text-gray-900 w-48">Status </th>
                   ...
                   <tbody class="divide-y divide-gray-200">
                     ...
                     <tr>
                         <td class="whitespace-nowrap px-4 py-2 text-gray-700 text-center">
                            {{ $jadwal->jenisPerawatan }} </td>
                         <td class="whitespace-nowrap px-4 py-2 text-gray-700 text-center">
                            {{ \Carbon\Carbon::parse($jadwal->tanggal)->format('d-m-Y') }}
                        </td>
                        <td class="whitespace-nowrap px-4 py-2 text-gray-700 text-center">{{ $jadwal->status }}</td>
                       <td class="whitespace-nowrap px-4 py-2 text-center">
                        <div class="flex justify-center gap-2">
                            <a href="{{ route('dashboard.karyawan.jadwal.ubah', $jadwal->id) }}">
                            ...
                            <button class="inline-flex items-center gap-2 rounded-md px-4 py-2 text-sm text-white shadow-sm hover:bg-green-100 focus:relative"
                                    onclick="confirmHapus('{{ route('dashboard.karyawan.jadwal.hapus', $jadwal->id) }}')">
                     ...
                     @empty
                     <tr>
                       <td colspan="5" class="whitespace-nowrap px-4 py-2 text-center text-gray-700">Tidak ada data jadwal</td>
                     </tr>
                     @endforelse
                   </tbody>
                 </table>
               ...
               <div class="flex flex-col items-center ml-20 mt-1">
                 ...
                 <span class="text-sm text-gray-700 dark:text-gray-400">
                     tampilkan <span class="font-semibold text-gray-900 dark:text-white">{{ $jadwals->firstItem() }}</span> sampai <span class="font-semibold text-gray-900 dark:text-white">{{ $jadwals->lastItem() }}</span> dari <span class="font-semibold text-gray-900 dark:text-white">{{ $jadwals->total() }}</span> Entries
                 </span>
                 <!-- Pagination -->
                 <ol class="flex justify-center gap-1 text-xs font-medium mt-2">
                     ...
                     @else
                         <li>
                             <a href="{{ $jadwals->previousPageUrl() }}" class="inline-flex items-center justify-center px-3 py-2 rounded border border-gray-100 bg-white text-gray-900 rtl:rotate-180 hover:bg-gray-100">
                                 <span class="sr-only">Prev Page</span>
                                 <svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                                     <path fill-rule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd" />
                                 </svg>
                     ...
                     @if ($jadwals->hasMorePages())
                         <li>
                             <a href="{{ $jadwals->nextPageUrl() }}" class="inline-flex items-center justify-center px-3 py-2 rounded border border-gray-100 bg-white text-gray-900 rtl:rotate-180 hover:bg-gray-100">
